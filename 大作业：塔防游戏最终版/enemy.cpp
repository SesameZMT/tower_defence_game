...
const QSize Enemy::m_fixedSize(55,44);

Enemy::Enemy(wayPoint * startWayPoint,MainWindow *game,int walkingSpeed,int maxHp,QString path) :
    QObject(0),
    m_maxHp(maxHp),
...
Enemy::~Enemy()
{
    ...
    m_destinationWayPoint=nullptr;//全部设为空指针
    m_game=nullptr;
}
...
void Enemy::draw(QPainter *painter) const
{
    ...
    painter->drawPixmap(tmp.x(),tmp.y(),m_fixedSize.width(), m_fixedSize.height(),m_path);
    painter->restore();
}
...
void Enemy::getAttacked(QPointer<Tower> tower)
{
...
void Enemy::getRemoved()
{
    ...
    {
        m_game->removeEnemy(this);
        foreach(QPointer<Tower> tower,m_attackerTowerList)
                    tower->targetKilled();
    }
}

void Enemy::getLostSight(QPointer<Tower> tower)
{
