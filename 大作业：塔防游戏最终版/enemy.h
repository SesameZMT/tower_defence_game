...
class Enemy : public QObject
{
    Q_OBJECT
public:
    Enemy(wayPoint * startPoint, MainWindow * game,int walkingSpeed,int maxHp,QString path);
    ~Enemy();
    ...
    void getAttacked(QPointer<Tower> tower);//被tower攻击
    void getDamaged(int damage);//敌人被攻击受到伤害
    void getRemoved();//当血量非正时，敌人死亡，需要移除
    void getLostSight(QPointer<Tower> tower);//敌人脱离tower的攻击范围
...
private:
    ...
    QPoint m_pos;//当前位置
    QString m_path;//图片路径
    QList<QPointer<Tower>> m_attackerTowerList;//正在攻击该敌人的防御塔list
